		...
		//} else {
			if (mission_plane_send) then {
				...
				if ((roleDescription player find "SpecOps") >= 0) then {
					_group = format["UnfinishedBusiness.core\data\images\fractions\%1\group_specops_logo.paa", _fraction];
					_header = localize format["%1_SPECOPS_ORDER_HEADER_01", _fraction];
					_signature = format["SCO 1201.SE\nMMEA-1\n%1 %2 %3", (_now select 2), (_now select 1), (_now select 0)];
					_text = format["%1\n\n%2: %3\n\n%4: %5\n\n%6: %7", localize format["%1_SPECOPS_BRIEFING_HEADER_01", _fraction], localize "MISSION", localize "WEST_BRIEFING_01", localize "OBJ_PRI", format [localize "WEST_SPECOPS_BRIEFING_02", D_LOCATION], localize "OBJ_SEC", localize "WEST_SPECOPS_BRIEFING_03"];
				} else {
