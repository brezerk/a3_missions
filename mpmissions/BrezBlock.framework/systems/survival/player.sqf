...
private _puke = {
	if ((player getVariable ['ACEX_field_rations_thirst', 0]) < 40.0) then {
		...
		playsound3d [_sound, player];
		player setVariable ['ACEX_field_rations_thirst', 65, true];
		if ((player getVariable ['ACEX_field_rations_hunger', 0]) < 40.0) then {
			player setVariable ['ACEX_field_rations_hunger', 70, true];
		};
		sleep 5;
...
private _hallucinations_01 = {
	...
	//for "_i" from 1 to ((random 1) + 1) do {
		...
		_effect ppEffectAdjust[1, (((random 3) + 1) * 0.1) , true];
...
Fn_Update_Temperature = {
	_rain = rain;
	_t_diff_base = -0.0002 + (linearConversion [0, 1, _rain, 0, -0.0005, true]);
	_t_local = 16;
	_img = "";
	...
	_t_diff_base = _t_diff_base + linearConversion [0, 20, speed player, 0, 0.0008, true];
	...
	if (bb_srv_temp_body >= 36.8) then {
		bb_srv_temp_body = 36.8;
	} else {
	...
	_img = format ["addons\BrezBlock.framework\data\survival\health%1.paa", round linearConversion [0, 0.8, damage player, 4, 0, true]];
	
	private _ctrl = uiNamespace getVariable "bb_survival_hud" displayCtrl 2995;
	if (!isNil "_ctrl") then { _ctrl ctrlSetText _img; };
	
	if ((bb_srv_dmg_chem > 0) || (bb_srv_dmg_rad > 0) ||(bb_srv_dmg_bac > 0)) then {
		if ((bb_srv_dmg_chem > 10) || (bb_srv_dmg_rad > 10) ||(bb_srv_dmg_bac > 10)) then {
			_img = "addons\BrezBlock.framework\data\survival\temp_down1.paa";
	...
	private _ctrl = uiNamespace getVariable "bb_survival_hud" displayCtrl 2997;
	if (!isNil "_ctrl") then { _ctrl ctrlSetText _img; };
...
Fn_Update_ACEX_Rations = {
	...
	private _status = player getVariable ['ACEX_field_rations_hunger', 0];
	...
	_status = player getVariable ['ACEX_field_rations_thirst', 0];
