...
if (!("rvg_antiRad" in (items _target))) then {
	_target removeItem "rvg_antiRad";
} else {
	if (!("rvg_antiRad" in (items _player))) exitWith {systemChat "Потрібен фізрозчин для інфузії.";};
	_player removeItem "rvg_antiRad";
};
...
[
	...
	[_target, _player],
	{
		...
		_target setVariable["bb_srv_derad", time, true];
